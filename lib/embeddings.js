...
/**
 * Generate an embedding for the given text and store it on the abstract row.
 * Designed to be called fire-and-forget (not awaited).
 *
 * @param {import("@supabase/supabase-js").SupabaseClient} supabase
 * @param {string} id   - abstract row ID
 * @param {string} text - text to embed (usually `title. abstract_text`)
 */
export async function embedAndStore(supabase, id, text) {
  try {
    const embedding = await generateEmbedding(text); // returns number[] of length 384
